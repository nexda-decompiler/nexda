...
void disassemble(const std::string& inputFile, const std::string& outputFile, bool pure) {
    int fd;
    Elf* elf;
    Elf_Scn* scn = NULL;
    GElf_Shdr shdr;
...
int main(int argc, char* argv[]) {
    if (argc < 4 || std::string(argv[2]) != "-o") {
        std::cerr << "Usage: " << argv[0] << " <input_file> -o <output_file> [-pure]" << std::endl;
        return 1;
    }
