...
public:
    Decompiler(const char* inputFile, const char* outputFile) : elf(nullptr), fd(-1) {
...
private:
    ...
    void disassembleSection(Elf_Scn* scn, GElf_Shdr* shdr, uint64_t startAddress) {
        ...
        cs_insn* insn;
        size_t count = cs_disasm(handle, code + offset, code_size - offset, startAddress, 0, &insn);
    ...
    void generatePseudoCode(const cs_insn& insn) {
        ...
        if (mnemonic == "mov") {
            outFile << op_str.substr(op_str.find(',') + 1) << " = " << op_str.substr(0, op_str.find(',')) << ";\n";
